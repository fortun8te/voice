#!/usr/bin/env python3
"""
VOICE — Moonshine Tiny transcription server.
...
def _load_mlx_audio():
    """Try mlx-audio backend (Metal-accelerated).
    API (mlx-audio >= 0.4.0):
        from mlx_audio.stt import load_model
        from mlx_audio.stt.generate import generate_transcription
        model = load_model("UsefulSensors/moonshine-tiny")
        result = generate_transcription(model=model, audio=audio_path, format="txt")

    Model priority:
      1. UsefulSensors/moonshine-tiny  — official weights
      2. mlx-community/moonshine-tiny  — community MLX conversion fallback
    """
    try:
        from mlx_audio.stt import load_model
        from mlx_audio.stt.generate import generate_transcription
        ...
        _model_candidates = [
            ...
            "mlx-community/moonshine-tiny",
        ]
        model = None
        _load_err = None
        for _mid in _model_candidates:
            try:
                print(f"[moonshine_server] trying {_mid} ...", file=sys.stderr, flush=True)
                model = load_model(_mid)
                print(f"[moonshine_server] loaded {_mid}", file=sys.stderr, flush=True)
                break
            except Exception as _e:
                print(f"[moonshine_server] {_mid} failed: {_e}", file=sys.stderr, flush=True)
                _load_err = _e
        if model is None:
        ...
        def _to_wav_if_needed(src):
            """Always re-encode to 16kHz mono PCM_16 WAV — Moonshine is trained
            on 16kHz mono audio and expects that input format."""
            if _sf is None:
            ...
            # Resample to 16kHz if needed (Moonshine's training sample rate).
            if sr != 16000:
                try:
                    ...
                    pass  # write at native sample rate, let Moonshine try
...
def main():
    # mlx-audio only — no transformers fallback (Moonshine lacks a useful transformers port).
        ...
        while True:
            ...
            try:
                ...
                if os.getppid() != parent:
                    print(f"[moonshine_server] parent {parent} died, exiting", file=sys.stderr, flush=True)
                    os._exit(0)
            except Exception:
                pass
