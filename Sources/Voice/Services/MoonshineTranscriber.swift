// VOICE — Moonshine Tiny Speech Transcriber
// ============================================================
// Manages a persistent Python subprocess (moonshine_server.py) that loads
...
//
// Falls back silently to nil if Python / moonshine is not installed.
// The entire triple-model path is optional — Parakeet v2 is always the fallback.
// ============================================================

import Foundation

actor MoonshineTranscriber {
    static let shared = MoonshineTranscriber()
    ...
    private func _deliverEOF() {
        ...
        print("[VOICE-Moonshine] Subprocess died — marking failed")
    }
    ...
    private static func scriptPath() -> String? {
        ...
        if let bundled = Bundle.main.path(forResource: "moonshine_server", ofType: "py") {
            return bundled
        }
        ...
        if let execURL = Bundle.main.executableURL {
            let candidate = execURL.deletingLastPathComponent()
                .appendingPathComponent("moonshine_server.py").path
    ...
    /// Start the Moonshine subprocess. Call once at app launch — non-blocking,
    /// failures are silent. Subsequent calls are no-ops.
    func start() async {
        guard case .idle = state else { return }
        state = .starting
        print("[VOICE-Moonshine] Starting server…")

        guard let python = Self.findPython() else {
            state = .unavailable("Python 3 not found (install via Homebrew)")
            print("[VOICE-Moonshine] Python not found — triple-model disabled")
            return
        }

        guard let script = Self.scriptPath() else {
            state = .unavailable("moonshine_server.py not bundled in app")
            print("[VOICE-Moonshine] Script not found — triple-model disabled")
            return
        }
        ...
        do {
            ...
            print("[VOICE-Moonshine] Launch failed: \(error)")
            return
        }
        ...
        guard let startupLine = await nextLine(timeout: 30),
            ...
            state = .failed("No startup handshake from Moonshine server")
            proc.terminate()
            print("[VOICE-Moonshine] No handshake — triple-model disabled")
            return
        }
        ...
        if status == "ready" {
            ...
            print("[VOICE-Moonshine] Unavailable: \(reason)")
        }
    }
    ...
    /// Transcribe an audio file via Moonshine. Returns nil if unavailable or timed out.
    /// Timeout is 8s — Moonshine Tiny on warm model is ~1-2s; allow extra for first inference.
    func transcribe(url: URL) async -> String? {
        ...
        do {
            ...
            print("[VOICE-Moonshine] Write failed: \(error) — subprocess likely dead")
        ...
        guard let responseLine = await nextLine(timeout: 8.0),
            ...
            print("[VOICE-Moonshine] Response timeout or bad JSON")
            return nil
        }
        ...
        if let text = json["text"] as? String, !text.isEmpty {
            ...
            print("[VOICE-Moonshine] ← '\(t.prefix(80))'")
            return t
        }
        if let error = json["error"] as? String {
            consecutiveErrors += 1
            print("[VOICE-Moonshine] Error (\(consecutiveErrors)/\(errorThreshold)): \(error)")
            if consecutiveErrors >= errorThreshold {
                print("[VOICE-Moonshine] Circuit breaker tripped — triple-model disabled this session")
            }
        }
        return nil
    }
    ...
    func shutdown() {
        if let proc = process, proc.isRunning {
            print("[VOICE-Moonshine] Shutting down subprocess (PID \(proc.processIdentifier))")
            proc.terminate()
        }
        process = nil
        stdinHandle = nil
